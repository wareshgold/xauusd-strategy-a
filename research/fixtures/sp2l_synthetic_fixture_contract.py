...
# Explicit research gate: all executable geometry dimensions remain blocked
# until a Tier-1/Tier-2 discriminator is manually adjudicated.
...
def make_fixture(
    ...
    labels = tuple(
        EvidenceLabel(
            ...
            note="Synthetic fixture only; source does not uniquely discriminate executable geometry.",
        )
...
EXAMPLE_BULLISH: Final[SyntheticFixture] = make_fixture(
    "synthetic-bullish-confirmed-semantics-only-v1",
    Direction.BULLISH,
    (
        Candle(0, "2026-01-01T00:00:00Z", 100.0, 101.0, 99.5, 100.5),
        Candle(1, "2026-01-01T00:01:00Z", 100.5, 103.0, 100.4, 102.7),
        Candle(2, "2026-01-01T00:02:00Z", 102.7, 104.0, 102.0, 103.5),
        Candle(3, "2026-01-01T00:03:00Z", 103.5, 103.8, 101.8, 102.4),
    ),
...
EXAMPLE_BEARISH: Final[SyntheticFixture] = make_fixture(
    "synthetic-bearish-confirmed-semantics-only-v1",
    Direction.BEARISH,
    (
        Candle(0, "2026-01-01T00:00:00Z", 200.0, 200.5, 199.0, 199.5),
        Candle(1, "2026-01-01T00:01:00Z", 199.5, 199.7, 197.0, 197.3),
        Candle(2, "2026-01-01T00:02:00Z", 197.3, 198.0, 196.0, 196.5),
        Candle(3, "2026-01-01T00:03:00Z", 196.5, 198.2, 196.3, 197.8),
    ),
    ("synthetic-only; no source frame asserted",),
)
