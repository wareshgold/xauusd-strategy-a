...
| F9 | Entry vs Leg-2 start | **PRIMARY-ARTIFACT PENDING-ENTRY CONCEPT + WORKED-TRADE EXECUTED POSITIONS CONFIRMED / ACTIVATION SEMANTICS UNRESOLVED** | No | BLOCKED |
...
| F11 | Pending-order replacement / refresh | **PRIMARY-ARTIFACT TEMPORAL PERSISTENCE + DELETE OBSERVED / WORKED-TRADE HISTORY SHOWS ORDERS AND CLOSES / LIFECYCLE CAUSALITY UNRESOLVED** | No | BLOCKED |
...
- ~66:30: a separate history row around **3269.88** has no SL/TP and a later close price around **3261.74**. This demonstrates that a displayed order can exist without SL/TP in the worked account history, but it does not establish why or when that order was created, whether it was a pending order before execution, or whether it was managed manually.
