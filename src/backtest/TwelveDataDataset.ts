...
export function twelveDataToDataset(data: TwelveDataResponse, timeframe: '1min'|'5min'): HistoricalDataset {
  ...
  const rows: Array<Candle> = data.values.map((v) => {
    ...
    const volume = v.volume === undefined ? undefined : Number(v.volume);
    if (volume !== undefined && !Number.isFinite(volume)) throw new Error(`Invalid Twelve Data volume at ${timestamp}`);
    return volume === undefined ? { timestamp, open, high, low, close } : { timestamp, open, high, low, close, volume };
