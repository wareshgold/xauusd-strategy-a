...
function pgH01(bars: readonly Ohlc[]): boolean { return bars.length >= 3 && bars[2].low > bars[0].high; }
function pgH02(bars: readonly Ohlc[]): boolean {
  if (bars.length < 3) return false;
  return bars[0].close > bars[0].open && bars[1].close > bars[1].open && bars[2].low > bars[0].high;
}
function pgH03(bars: readonly Ohlc[]): boolean { return bars.length >= 3 && bars[2].low > bars[0].high; }
function pgH04(bars: readonly Ohlc[]): boolean { return bars.length >= 3 && bars[2].low > bars[0].high && bars[1].low > bars[0].low; }

// Candidate A/B/C/D encodings — deliberately non-canonical.
function abcdH01(bars: readonly Ohlc[]): number { return bars.length < 2 ? 0 : bars[1].high - bars[0].low; }
function abcdH02(bars: readonly Ohlc[]): number { return bars.length < 2 ? 0 : bodyHigh(bars[1]) - bodyLow(bars[0]); }
function abcdH03(bars: readonly Ohlc[]): number { return bars.length < 2 ? 0 : bars[1].close - bars[0].close; }

// Candidate entry encodings — no candidate is asserted as source meaning.
function enH01(bars: readonly Ohlc[]): number { return bars.length < 1 ? 0 : bars[0].low; }
function enH02(bars: readonly Ohlc[]): number { return bars.length < 2 ? 0 : bars[1].low; }
function enH03(bars: readonly Ohlc[]): number { return bars.length < 1 ? 0 : (bars[0].low + bars[0].high) / 2; }

// Candidate SL encodings.
function slH01(bars: readonly Ohlc[]): number { return bars.length === 0 ? 0 : bars[0].low; }
function slH02(bars: readonly Ohlc[]): number { return bars.length === 0 ? 0 : bodyLow(bars[0]); }
...
function tpH01(bars: readonly Ohlc[]): number {
  ...
  return bars[2].close + (bars[1].high - bars[0].low);
}
function tpH04(bars: readonly Ohlc[]): number {
  if (bars.length < 3) return 0;
  const entry = bars[2].close;
  return entry + (entry - bars[0].low);
}
...
function evaluate(fixture: G380Fixture, hypothesis: string): number | boolean {
  switch (hypothesis) {
    case 'PG-H01': return pgH01(fixture.bars); case 'PG-H02': return pgH02(fixture.bars); case 'PG-H03': return pgH03(fixture.bars); case 'PG-H04': return pgH04(fixture.bars);
    case 'ABCD-H01': return abcdH01(fixture.bars); case 'ABCD-H02': return abcdH02(fixture.bars); case 'ABCD-H03': return abcdH03(fixture.bars);
    case 'EN-H01': return enH01(fixture.bars); case 'EN-H02': return enH02(fixture.bars); case 'EN-H03': return enH03(fixture.bars);
    case 'SL-H01': return slH01(fixture.bars); case 'SL-H02': return slH02(fixture.bars);
    case 'TP-H01': return tpH01(fixture.bars); case 'TP-H04': return tpH04(fixture.bars);
...
export function runG380(): readonly G380Observation[] {
  return G380_FIXTURES.map((fixture) => {
    const outputA = evaluate(fixture, fixture.hypothesisA); const outputB = evaluate(fixture, fixture.hypothesisB);
    return { id:fixture.id, family:fixture.family, hypothesisA:fixture.hypothesisA, hypothesisB:fixture.hypothesisB, outputA, outputB, classification:Object.is(outputA, outputB) ? 'EQUIVALENT' : 'DISTINCT', canonical:false };
  });
}

export function g380ExpectedClassificationsMatch(): boolean {
  return runG380().every((result) => G380_FIXTURES.find((f) => f.id === result.id)?.expected === result.classification);
}
