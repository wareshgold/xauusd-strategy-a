...
function path(r,candles){
  ...
  if(!(risk>0&&tp1Distance>0))return null;
  if(!(buy?tp1>entry:tp1<entry))return null;
  ...
  for(let i=ei+1;i<candles.length;i++){
    ...
    if(tp1Idx!==null&&slIdx!==null)break;
  }
  if(tp1Idx===null||(slIdx!==null&&slIdx<tp1Idx))return null;
  const tp1R=tp1Distance/risk;
  // Data-contract guard: TP2 must be a directional PRICE target, not an R-multiple/distance.
  // A TP2 more than 4x the TP1 price excursion is rejected as malformed for this study.
  const tp2Directional=tp2!==null&&(buy?tp2>tp1:tp2<tp1);
  const tp2Distance=tp2===null?null:Math.abs(tp2-entry);
  const validTp2=tp2Directional&&finite(tp2Distance)&&tp2Distance>0&&tp2Distance<=tp1Distance*4;
  const tp2R=validTp2?tp2Distance/risk:null;
  let tp2Idx=null,protectedIdx=null;
  if(validTp2){
    for(let i=tp1Idx+1;i<Math.min(candles.length,tp1Idx+1+HORIZON);i++){
      const c=candles[i],hi=Number(c.high),lo=Number(c.low);if(!finite(hi)||!finite(lo))continue;
      if(tp2Idx===null&&(buy?hi>=tp2:lo<=tp2))tp2Idx=i;
      if(protectedIdx===null&&(buy?lo<=entry:hi>=entry))protectedIdx=i;
      if(tp2Idx!==null||protectedIdx!==null)break;
    }
  }
  return{entryTime:r.entryTime,direction:r.direction,tp1R,tp2R,barsToTp1:tp1Idx-ei,barsToTp2:tp2Idx===null?null:tp2Idx-tp1Idx,beIdx:protectedIdx===null?null:protectedIdx-tp1Idx,tp2Valid:validTp2};
}
function evaluate(p,name){
  if(name==='TP1_FULL_EXIT')return p.tp1R;
  if(!p.tp2Valid)return null;
  const half=p.tp1R*.5;
...
async function run(tf){
  ...
  const s=split(paths);
  const report={strategy:'Strategy A / SP2L',mode:'RESEARCH_TP1_MANAGEMENT_SCENARIOS_GLOBAL_HOLDOUT_V2',timeframe:tf,scope:'Post-TP1 management scenarios; entry generation is unchanged. A signal remains valid until TP1 or structural invalidation.',methodology:{tp1:'minimum objective; all scenarios require TP1 first',runner:'50% position remains after TP1 in runner scenarios',tp2:'TP2 is used only when present as a validated directional price target',window:`${HORIZON} candles after TP1`,protectedStop:'BE means entry price; +0.25R means entry plus 0.25 initial-risk in the favorable direction',selection:'No optimization; scenarios fixed before holdout',warning:'The 4x TP1-distance check is a data-contract sanity guard, not a performance optimization.'},counts:{tp1Qualified:paths.length,tp2Valid:paths.filter(p=>p.tp2Valid).length,dev:s.dev.length,validation:s.validation.length,holdout:s.holdout.length},scenarios:{}};
