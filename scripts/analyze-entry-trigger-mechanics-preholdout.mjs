...
function bucket(rows, key, ranges) {
...
async function reconstruct() {
  ...
  for (const trade of baseline.trades ?? []) {
    ...
    for (const b of breakouts) {
      ...
      matched = { b, ft, spike, correction, trigger };
      break;
    }
    if (!matched) continue;
...
const report = {
  ...
  mode: 'RESEARCH_ENTRY_TRIGGER_MECHANICS_PREHOLDOUT_V1',
  timeframe: TIMEFRAME,
  ...
  byDelay: bucket(rows, 'delayBars', [['0', r => r.delayBars === 0], ['1', r => r.delayBars === 1], ['2', r => r.delayBars === 2], ['3-5', r => r.delayBars >= 3 && r.delayBars <= 5], ['6+', r => r.delayBars >= 6]]),
  byCorrectionDepthBars: bucket(rows, 'correctionDepthBars', [['1', r => r.correctionDepthBars === 1], ['2', r => r.correctionDepthBars === 2], ['3', r => r.correctionDepthBars === 3], ['4+', r => r.correctionDepthBars >= 4]]),
  byTriggerExtensionPctSpike: bucket(rows, 'triggerExtensionPctSpike', [['<10%', r => r.triggerExtensionPctSpike < .10], ['10-25%', r => r.triggerExtensionPctSpike >= .10 && r.triggerExtensionPctSpike < .25], ['25-50%', r => r.triggerExtensionPctSpike >= .25 && r.triggerExtensionPctSpike < .50], ['>50%', r => r.triggerExtensionPctSpike >= .50]]),
...
console.log(`${TIMEFRAME}: matched=${rows.length} winners=${winners.length} losers=${losers.length} avgR=${report.overall.avgR.toFixed(4)} PF=${report.overall.PF?.toFixed(4) ?? 'n/a'}`);
