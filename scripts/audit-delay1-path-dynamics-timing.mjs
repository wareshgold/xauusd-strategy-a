...
function auditMetric(rows, key) {
  const vals = rows.map(r => Number(r.metrics?.[key])).filter(Number.isFinite);
...
for (const tf of ['1min','5min']) {
  ...
  for(const field of ['timeToMae25Median','timeToMae50Median','timeToMae75Median','timeToMae1Median','timeToMfe05Median','timeToMfe1Median','timeToMfe2Median']) {
...
console.log('\nAudit corrected: report path and aggregate timing-field names now match the analyzer; diagnostic only.');
