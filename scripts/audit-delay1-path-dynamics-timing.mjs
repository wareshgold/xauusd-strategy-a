...
function auditMetric(rows, key) {
  ...
  return {
    ...
    max: Math.max(...vals),
    mean: mean(vals),
    shareEqual1: vals.filter(x=>Math.abs(x-1)<1e-9).length/vals.length,
    shareInteger: vals.filter(x=>Number.isInteger(x)).length/vals.length,
  };
}
...
function auditWithinCheckpoint(report, field) {
  ...
  for (const w of report.windows ?? []) {
    for (const [name,m] of Object.entries(w.metrics ?? {})) {
