...
async function run(timeframe){
 ...
 // Use the baseline backtest's realized trade outcomes as the sole outcome source.
 // Join by exact entryIndex + direction + entry/SL/TP to avoid recomputing outcomes here.
 ...
 console.log(`${timeframe}: baselinePreHoldout=${baselineTrades.length} forensicSelected=${selected.filter(c=>c.index<cutoffIndex).length} matched=${rows.length} unmatched=${unmatched.length} matchRate=${(reportOut.parity.matchRate*100).toFixed(2)}%`);
