...
function outcomeOnPath(candles, row) {
  ...
  for (let i = row.entryIndex + 1; i < candles.length; i += 1) {
    ...
    if (hitTP) return { result: 'TP1', index: i, timestamp: c.timestamp, rMultiple: Math.abs(tp1 - entry) / risk };
  }
...
async function main() {
  ...
  const cases = (report.cases ?? []).map((raw) => {
    const entryCandle = candles[raw.entryIndex];
    if (!entryCandle) throw new Error(`Missing historical candle at entryIndex ${raw.entryIndex} for ${raw.entryTime}`);
    if (entryCandle.timestamp !== raw.entryTime) {
      throw new Error(`Historical candle lineage mismatch at entryIndex ${raw.entryIndex}: report=${raw.entryTime} candles=${entryCandle.timestamp}`);
    }

    // Path Geometry V2 intentionally stores geometry flattened under each case, but
    // the original entry/SL/TP fields are not persisted there. Reconstruct them from
    // canonical entry-close + descriptive stop/reward distances for this SELL-only study.
    const entry = Number(entryCandle.close);
    const stopDistance = Number(raw.stopDistance);
    const rewardDistance = Number(raw.rewardDistance);
    if (!(Number.isFinite(entry) && stopDistance > 0 && rewardDistance > 0)) {
      throw new Error(`Missing reconstructable trade geometry for ${raw.entryTime}`);
    }
    const row = {
      ...raw,
      direction: raw.direction ?? 'SELL',
      entry,
      stopLoss: entry + stopDistance,
      tp1: entry - rewardDistance,
    };
    ...
    if (outcome.result === 'INVALID' || outcome.result === 'OPEN') {
      throw new Error(`Unexpected recomputed outcome ${outcome.result} for ${row.entryTime}`);
    }
    ...
    return {
      ...
      geometry: Object.fromEntries(Object.entries(raw).filter(([k]) => !['entryTime', 'entryIndex', 'r'].includes(k))),
    };
  });
  ...
  const reportOut = {
    ...
    methodology: {
      ...
      outcome: 'Outcome is recomputed directly from the canonical historical candles using the same first-hit SL/TP semantics as the deterministic backtest. Trade geometry is reconstructed from the canonical entry candle close plus the report stop/reward distances.',
