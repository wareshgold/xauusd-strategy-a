...
const phase27Cases=[...(PH27.cases??[])];
const byTime=new Map(phase27Cases.map(x=>[x.entryTime,x]));
...
const report={strategy:BASELINE.strategy,mode:'PHASE33_COUNTERFACTUAL_PHASE27_ARCHETYPE_DECOMPOSITION',timeframe:'5min',baselineCommit:BASELINE_COMMIT,generationParity:{stored:stored.length,generated:generated.length,exact},phase27Mapping:{mapped:rows.length,unmapped:stored.length-rows.length,sourceCases:phase27Cases.length,dCases:phase27Cases.filter(x=>x.archetype===D).length,nonDCases:phase27Cases.filter(x=>x.archetype!==D).length,spikeQ66:qSpike,correctionEfficiencyQ66:qCorr},overall:stats(rows),byArchetype:[group('D',x=>x.archetype===D),group('NON_D',x=>x.archetype!==D)],byOriginAndArchetype:[group('BUY_ORIGIN_D',x=>x.originalDirection==='BUY'&&x.archetype===D),group('BUY_ORIGIN_NON_D',x=>x.originalDirection==='BUY'&&x.archetype!==D),group('SELL_ORIGIN_D',x=>x.originalDirection==='SELL'&&x.archetype===D),group('SELL_ORIGIN_NON_D',x=>x.originalDirection==='SELL'&&x.archetype!==D)],geometry:[group('D_SPIKE_HIGH',x=>x.archetype===D&&x.spikeHigh),group('D_CORR_EFF_HIGH',x=>x.archetype===D&&x.correctionEffHigh),group('D_SPIKE_HIGH_CORR_EFF_HIGH',x=>x.archetype===D&&x.spikeHigh&&x.correctionEffHigh),group('NON_D_SPIKE_HIGH_CORR_EFF_HIGH',x=>x.archetype!==D&&x.spikeHigh&&x.correctionEffHigh)],methodology:{researchOnly:true,noOptimization:true,noTradingRules:true,archetypeSource:'Phase27 D-archetype report; all archetype and geometry features are taken from the same Phase27 case keyed by canonical entryTime.',directionSource:'Baseline candidate generation; direction is counterfactually mirrored with preserved absolute risk/reward.',note:'Only baseline trades present in the Phase27 case population can be mapped; unmapped baseline trades remain excluded from archetype-conditioned groups.'},status:'COUNTERFACTUAL_ONLY_DECOMPOSITION'};
