...
const qSpike=PH27.interactionConditioning[0].thresholds.spikeSizeR;
const qCorr=PH27.interactionConditioning[1].thresholds.correctionEfficiency;
...
function evalFlip(c){const risk=Math.abs(c.entry-c.stopLoss);for(let i=c.entryIndex+1;i<candles.length;i++){const x=candles[i];const sl=c.direction==='BUY'?x.low<=c.stopLoss:x.high>=c.stopLoss;const tp=x.direction==='BUY'?x.high>=c.tp1:x.low<=c.tp1;if(sl&&tp)return null;if(sl)return -1;if(tp)return Math.abs(c.tp1-c.entry)/risk;}return null;}
...
for(let index=0;index<candles.length;index++){
 ...
 for(const spike of spikes.candidates.filter(s=>s.endIndex<index)){
  ...
  generated.push({entryIndex:index,entryTime:trigger.timestamp,direction:trigger.direction,entry:trigger.entryPrice,stopLoss:invalidation.invalidationLevel,tp1:projection.tp1,session:session.session,qualityGrade:quality.grade,spikeSizeR:spike.spikeSizeR,correctionEfficiency:correction.correctionEfficiency}); break;
 }
}
const stored=BASELINE.trades??[];const rows=[];let exact=0;
for(const t of stored){const g=generated.find(x=>x.entryTime===t.entryTime);if(!g||g.entryIndex!==t.entryIndex||g.direction!==t.direction||!same(g.entry,t.entry)||!same(g.stopLoss,t.stopLoss)||!same(g.tp1,t.tp1))throw new Error(`Generation parity failed at ${t.entryTime}`);exact++;const p=byTime.get(t.entryTime);if(!p)continue;const risk=Math.abs(g.entry-g.stopLoss),reward=Math.abs(g.tp1-g.entry);const flipped={...g,direction:g.direction==='BUY'?'SELL':'BUY',stopLoss:g.entry+(g.direction==='BUY'?risk:-risk),tp1:g.entry+(g.direction==='BUY'?-reward:reward)};rows.push({r:evalFlip(flipped),originalDirection:g.direction,archetype:p.archetype,spikeHigh:g.spikeSizeR>=qSpike,correctionEffHigh:g.correctionEfficiency>=qCorr,entryTime:g.entryTime});}
...
const report={strategy:BASELINE.strategy,mode:'PHASE33_COUNTERFACTUAL_PHASE27_ARCHETYPE_DECOMPOSITION',timeframe:'5min',baselineCommit:BASELINE_COMMIT,generationParity:{stored:stored.length,generated:generated.length,exact},phase27Mapping:{mapped:rows.length,unmapped:stored.length-rows.length,spikeQ66:qSpike,correctionEfficiencyQ66:qCorr},overall:stats(rows),byArchetype:[group('D',x=>x.archetype===D),group('NON_D',x=>x.archetype!==D)],byOriginAndArchetype:[group('BUY_ORIGIN_D',x=>x.originalDirection==='BUY'&&x.archetype===D),group('BUY_ORIGIN_NON_D',x=>x.originalDirection==='BUY'&&x.archetype!==D),group('SELL_ORIGIN_D',x=>x.originalDirection==='SELL'&&x.archetype===D),group('SELL_ORIGIN_NON_D',x=>x.originalDirection==='SELL'&&x.archetype!==D)],geometry:[group('D_SPIKE_HIGH',x=>x.archetype===D&&x.spikeHigh),group('D_CORR_EFF_HIGH',x=>x.archetype===D&&x.correctionEffHigh),group('D_SPIKE_HIGH_CORR_EFF_HIGH',x=>x.archetype===D&&x.spikeHigh&&x.correctionEffHigh),group('NON_D_SPIKE_HIGH_CORR_EFF_HIGH',x=>x.archetype!==D&&x.spikeHigh&&x.correctionEffHigh)],methodology:{researchOnly:true,noOptimization:true,noTradingRules:true,archetypeSource:'Phase24 loss archetypes; predictors pre-entry only; direction is counterfactually mirrored with preserved absolute risk/reward.',note:'Only the 163 baseline losing cases present in Phase24 can be mapped to archetype labels; the remaining 69 baseline trades are intentionally excluded from archetype-conditioned groups.'},status:'COUNTERFACTUAL_ONLY_DECOMPOSITION'};
await mkdir(OUT,{recursive:true});await writeFile(resolve(OUT,'5min.json'),JSON.stringify(report,null,2));
console.log(`PHASE33_COUNTERFACTUAL_PHASE27_ARCHETYPE_DECOMPOSITION GENERATION_STORED=${stored.length} GENERATED=${generated.length} EXACT=${exact} MAPPED=${rows.length} UNMAPPED=${stored.length-rows.length}`);
for(const x of report.byArchetype)console.log(`${x.name} N=${x.n} WR=${(x.winRate*100).toFixed(2)}% avgR=${x.avgR?.toFixed(6)} PF=${x.profitFactor?.toFixed(6)} totalR=${x.totalR.toFixed(6)}`);
for(const x of report.byOriginAndArchetype)console.log(`${x.name} N=${x.n} WR=${(x.winRate*100).toFixed(2)}% avgR=${x.avgR?.toFixed(6)} PF=${x.profitFactor?.toFixed(6)} totalR=${x.totalR.toFixed(6)}`);
for(const x of report.geometry)console.log(`${x.name} N=${x.n} WR=${(x.winRate*100).toFixed(2)}% avgR=${x.avgR?.toFixed(6)} PF=${x.profitFactor?.toFixed(6)} totalR=${x.totalR.toFixed(6)}`);
