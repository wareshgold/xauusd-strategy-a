...
async function run(timeframe){
 ...
 const rejection={},accepted=[],triggerCandidates=[],correctionCandidates=[];
 for(let index=0;index<candles.length;index++){
  ...
  for(const spike of eligible){
   ...
   correctionCandidates.push({index,spikeEndIndex:spike.endIndex,correctionExtremeIndex:correction.correctionExtremeIndex,direction:correction.direction,depth:Math.abs(correction.extremePrice-spike.startPrice)/Math.max(Math.abs(spike.endPrice-spike.startPrice),1e-9)});
