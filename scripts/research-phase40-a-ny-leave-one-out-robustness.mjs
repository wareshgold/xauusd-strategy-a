...
const OUT=resolve(ROOT,'data/reports/strategy-a-phase40-a-ny-leave-one-out-robustness');
...
function same(a,b){return Number.isFinite(a)&&Number.isFinite(b)&&Math.abs(a-b)<=1e-9;}
...
const leaveOneOut=rows.map((removed,i)=>{const kept=rows.filter((_,j)=>j!==i);return{removedIndex:i+1,removedEntryTime:removed.entryTime,removedR:removed.r,...stats(kept.map(x=>x.r))};});
const avgRValues=leaveOneOut.map(x=>x.avgR),pfValues=leaveOneOut.map(x=>x.PF).filter(Number.isFinite),totalRValues=leaveOneOut.map(x=>x.totalR);
const positiveAfterRemoval=leaveOneOut.filter(x=>x.totalR>0).length,pfAbove1=leaveOneOut.filter(x=>Number.isFinite(x.PF)&&x.PF>1).length;
const report={strategy:BASELINE.strategy,mode:'PHASE40_A_NY_LEAVE_ONE_OUT_ROBUSTNESS',timeframe:'5min',baselineCommit:BASELINE_COMMIT,generationParity:{stored:stored.length,generated:generated.length,exact},scope:{archetype:'LOSS_A_NO_PRE_FAVORABLE',direction:'Original BUY flipped to SELL',session:'NEW_YORK',counterfactual:true},overall,leaveOneOut,robustnessSummary:{nRows:rows.length,positiveTotalRAfterSingleRemoval:positiveAfterRemoval,positiveTotalRRate:positiveAfterRemoval/rows.length,pfAbove1AfterSingleRemoval:pfAbove1,pfAbove1Rate:pfAbove1/rows.length,minAvgRAfterRemoval:Math.min(...avgRValues),maxAvgRAfterRemoval:Math.max(...avgRValues),minPFAfterRemoval:Math.min(...pfValues),maxPFAfterRemoval:Math.max(...pfValues),minTotalRAfterRemoval:Math.min(...totalRValues),maxTotalRAfterRemoval:Math.max(...totalRValues)},methodology:{researchOnly:true,noOptimization:true,noTradingRules:true,purpose:'Leave-one-out sensitivity audit of the A/New York counterfactual edge.'},status:'COUNTERFACTUAL_ONLY_LEAVE_ONE_OUT_ROBUSTNESS_AUDIT'};
await mkdir(OUT,{recursive:true});await writeFile(resolve(OUT,'5min.json'),JSON.stringify(report,null,2));
console.log(`PHASE40_A_NY_LEAVE_ONE_OUT_ROBUSTNESS GENERATION_STORED=${stored.length} GENERATED=${generated.length} EXACT=${exact}`);
...
console.log(`LOO_POSITIVE_TOTALR=${positiveAfterRemoval}/${rows.length} LOO_PF_GT1=${pfAbove1}/${rows.length}`);
console.log(`LOO_AVGR_MIN=${Math.min(...avgRValues).toFixed(6)} MAX=${Math.max(...avgRValues).toFixed(6)}`);
console.log(`LOO_PF_MIN=${Math.min(...pfValues).toFixed(6)} MAX=${Math.max(...pfValues).toFixed(6)}`);
console.log(`LOO_TOTALR_MIN=${Math.min(...totalRValues).toFixed(6)} MAX=${Math.max(...totalRValues).toFixed(6)}`);
for(const x of leaveOneOut)console.log(`REMOVE ${x.removedEntryTime} R=${x.removedR.toFixed(6)} -> N=${x.n} avgR=${x.avgR.toFixed(6)} PF=${x.PF.toFixed(6)} totalR=${x.totalR.toFixed(6)}`);
console.log(`REPORT=${resolve(OUT,'5min.json')}`);console.log('STATUS=COUNTERFACTUAL_ONLY_LEAVE_ONE_OUT_ROBUSTNESS_AUDIT');
