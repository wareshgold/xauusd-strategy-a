...
const BUCKETS=[
  ['0-0.25R',0,0.25],['0.25-0.5R',0.25,0.5],['0.5-0.75R',0.5,0.75],['0.75-1R',0.75,1],['>1R',1,Infinity]
];
...
function build(candles,index){
  ...
  for(const spike of sp.candidates){
    ...
    const trigger=detectEntryTrigger(v,correction); if(!trigger||trigger.index!==index)continue;
    ...
    out.push({entryIndex:index,entryTime:trigger.timestamp,direction:trigger.direction,entry:trigger.entryPrice,stopLoss:inv.invalidationLevel,tp1:projection.tp1,session:session.session,correction,spike});
  }
  return out;
}

function pathFor(c,t){
  const i=t.entryIndex,e=t.entry,s=t.stopLoss,d=t.direction,r=Math.abs(e-s); if(!r)return null;
  const p=[]; let maxA=0,maxF=0;
  for(let j=i+1;j<=Math.min(c.length-1,i+20);j++){
    const f=(d==='BUY'?c[j].high-e:e-c[j].low)/r, a=(d==='BUY'?e-c[j].low:c[j].high-e)/r;
    maxF=Math.max(maxF,f); maxA=Math.max(maxA,a); p.push({b:j-i,f,a});
  }
  const first=(level,type)=>{for(const x of p){if((type==='f'?x.f:x.a)>=level)return x.b}return null};
  const advBefore=(favBar)=>favBar===null?maxA:Math.max(0,...p.filter(x=>x.b<=favBar).map(x=>x.a));
  return {maxF,maxA,firstFav:Object.fromEntries(FAV.map(x=>[x,first(x,'f')])),firstAdv1:first(1,'a'),advBeforePlus:Object.fromEntries(FAV.map(x=>[x,advBefore(first(x,'f'))])),horizons:Object.fromEntries(H.map(h=>{const q=p.filter(x=>x.b<=h);const fp=first(1,'f'),fa=first(1,'a');return[h,{mfe:Math.max(0,...q.map(x=>x.f)),mae:Math.max(0,...q.map(x=>x.a)),favFirst:fp!==null&&(fa===null||fp<fa),advFirst:fa!==null&&(fp===null||fa<fp),sameBar:fp!==null&&fa!==null&&fp===fa,plusReach:fp!==null,minusReach:fa!==null}]}))};
}

function bucketSummary(rows){
  const out={};
  for(const [name,lo,hi] of BUCKETS){
    const x=rows.filter(r=>r.adverseBudget>=lo&&r.adverseBudget<hi);
    out[name]={n:x.length,rate:pct(x.length,rows.length),avgR:mean(x.map(r=>r.rMultiple)),pf:pf(x.map(r=>r.rMultiple)),winRate:pct(x.filter(r=>r.rMultiple>0).length,x.length),medianMFE:median(x.map(r=>r.horizons.h20.mfe)),medianMAE:median(x.map(r=>r.horizons.h20.mae)),plus1First:pct(x.filter(r=>r.horizons.h20.favFirst).length,x.length),minus1First:pct(x.filter(r=>r.horizons.h20.advFirst).length,x.length),sameBar:pct(x.filter(r=>r.horizons.h20.sameBar).length,x.length)};
  }
  return out;
}

function summarize(rows){
  return {n:rows.length,avgR:mean(rows.map(r=>r.rMultiple)),pf:pf(rows.map(r=>r.rMultiple)),winRate:pct(rows.filter(r=>r.rMultiple>0).length,rows.length),medianMFEH20:median(rows.map(r=>r.horizons.h20.mfe)),medianMAEH20:median(rows.map(r=>r.horizons.h20.mae)),budgetBuckets:bucketSummary(rows),byFavMilestone:Object.fromEntries(FAV.map(level=>{const x=rows.filter(r=>r.adverseByFav[level]!==null);const b=Object.fromEntries(BUCKETS.map(([name,lo,hi])=>{const y=x.filter(r=>r.adverseByFav[level]>=lo&&r.adverseByFav[level]<hi);return[name,{n:y.length,avgR:mean(y.map(r=>r.rMultiple)),pf:pf(y.map(r=>r.rMultiple)),winRate:pct(y.filter(r=>r.rMultiple>0).length,y.length),plus2Reach:pct(y.filter(r=>r.horizons.h20.plus2Reach).length,y.length)}]}));return[`+${level}R`,{n:x.length,buckets:b}]})),horizons:Object.fromEntries(H.map(h=>{const x=rows.map(r=>r.horizons[h]);return[`h${h}`,{plus1ReachRate:pct(x.filter(v=>v.plusReach).length,x.length),minus1ReachRate:pct(x.filter(v=>v.minusReach).length,x.length),favorableFirst:pct(x.filter(v=>v.favFirst).length,x.length),adverseFirst:pct(x.filter(v=>v.advFirst).length,x.length),sameBar:pct(x.filter(v=>v.sameBar).length,x.length),medianMFE:median(x.map(v=>v.mfe)),medianMAE:median(x.map(v=>v.mae))}]}))};
}
...
async function run(tf){
  ...
  for(let i=0;i<PRE;i++){
    ...
    const p=pathFor(c,t); if(!p)continue;
    rows.push({entryIndex:i,rMultiple:Number(t.rMultiple),direction:t.direction,session:t.session,adverseBudget:p.advBeforePlus[1],adverseByFav:Object.fromEntries(FAV.map(x=>[x,p.advBeforePlus[x]])),horizons:p.horizons});
  }
  const dev=rows.filter(r=>r.entryIndex<DEV),val=rows.filter(r=>r.entryIndex>=DEV&&r.entryIndex<PRE);
  const report={strategy:'Strategy A',mode:'DELAY1_EXCURSION_BUDGET_OUTCOME',timeframe:tf,scope:{preHoldoutCandles:PRE,devCandles:DEV,valCandles:PRE-DEV,delayExactly:1,freshHoldoutExcluded:true},methodology:{buckets:BUCKETS.map(([name,lo,hi])=>({name,minR:lo,maxR:hi})),favMilestones:FAV,horizons:H,noOptimization:true,noFreshHoldout:true,productionUntouched:true,note:'Adverse budget is measured from entry until the first specified favorable milestone; trades that never reach the milestone are excluded from that milestone table. OHLC same-bar order remains ambiguous.'},DEV:summarize(dev),VAL:summarize(val),allPreHoldout:summarize(rows),rows};
  await mkdir(OUT,{recursive:true}); const out=resolve(OUT,`${tf}.json`); await writeFile(out,JSON.stringify(report,null,2));
  const print=(label,s)=>{console.log(`${label}: n=${s.n} AvgR=${s.avgR?.toFixed(3)} PF=${s.pf?.toFixed(3)} WR=${(100*s.winRate).toFixed(1)} MFE/MAE H20=${s.medianMFEH20?.toFixed(2)}/${s.medianMAEH20?.toFixed(2)}`);for(const[k,v]of Object.entries(s.budgetBuckets))console.log(`  ${k}: n=${v.n} rate=${(100*v.rate).toFixed(1)} AvgR=${v.avgR?.toFixed(3)} PF=${v.pf?.toFixed(3)} WR=${v.winRate===null?'null':(100*v.winRate).toFixed(1)} +1Rfirst=${v.plus1First===null?'null':(100*v.plus1First).toFixed(1)} -1Rfirst=${v.minus1First===null?'null':(100*v.minus1First).toFixed(1)} same=${v.sameBar===null?'null':(100*v.sameBar).toFixed(1)}`);};
