...
function build(candles,index){
  ...
  for(const spike of sp.candidates){
    ...
    out.push({entryIndex:index,entryTime:trigger.timestamp,direction:trigger.direction,entry:trigger.entryPrice,stopLoss:inv.invalidationLevel,tp1:projection.tp1,session:session.session});
  }
  return out;
}
...
function pathFor(c,t){
  ...
  const advBefore=(favBar)=>favBar===null?Math.max(0,...p.map(x=>x.a)):Math.max(0,...p.filter(x=>x.b<=favBar).map(x=>x.a));
  const firstFav=Object.fromEntries(FAV.map(x=>[x,first(x,'f')]));
  const advBeforePlus=Object.fromEntries(FAV.map(x=>[x,advBefore(firstFav[x])]));
  const horizons=Object.fromEntries(H.map(h=>{
    const q=p.filter(x=>x.b<=h), fp=first(1,'f'), fa=first(1,'a'), fp2=first(2,'f');
    return [h,{mfe:Math.max(0,...q.map(x=>x.f)),mae:Math.max(0,...q.map(x=>x.a)),favFirst:fp!==null&&(fa===null||fp<fa),advFirst:fa!==null&&(fp===null||fa<fp),sameBar:fp!==null&&fa!==null&&fp===fa,plusReach:fp!==null,plus2Reach:fp2!==null,minusReach:fa!==null}];
  }));
  return {firstFav,advBeforePlus,horizons};
}
...
function summarize(rows){
  return {
    n:rows.length,avgR:mean(rows.map(r=>r.rMultiple)),pf:pf(rows.map(r=>r.rMultiple)),winRate:pct(rows.filter(r=>r.rMultiple>0).length,rows.length),
    medianMFEH20:median(rows.map(r=>r.horizons.h20.mfe)),medianMAEH20:median(rows.map(r=>r.horizons.h20.mae)),
    budgetRelativeToPlus1R:bucketSummary(rows,'adverseBudget'),
    byFavMilestone:Object.fromEntries(FAV.map(level=>[`+${level}R`,{n:rows.filter(r=>r.adverseByFav[level]!==null).length,buckets:bucketSummary(rows,`adverseByFav_${level}`)}])),
    horizons:Object.fromEntries(H.map(h=>{const x=rows.map(r=>r.horizons[h]);return[`h${h}`,{plus1ReachRate:pct(x.filter(v=>v.plusReach).length,x.length),minus1ReachRate:pct(x.filter(v=>v.minusReach).length,x.length),favorableFirst:pct(x.filter(v=>v.favFirst).length,x.length),adverseFirst:pct(x.filter(v=>v.advFirst).length,x.length),sameBar:pct(x.filter(v=>v.sameBar).length,x.length),medianMFE:median(x.map(v=>v.mfe)),medianMAE:median(x.map(v=>v.mae))}]}))
  };
}
...
async function run(tf){
  ...
  for(let i=0;i<PRE;i++){
    ...
    rows.push({entryIndex:i,rMultiple:Number(t.rMultiple),adverseBudget:p.advBeforePlus[1],adverseByFav:Object.fromEntries(FAV.map(x=>[x,p.advBeforePlus[x]])),...Object.fromEntries(FAV.map(x=>[`adverseByFav_${x}`,p.advBeforePlus[x]])),horizons:p.horizons});
  }
  const dev=rows.filter(r=>r.entryIndex<DEV),val=rows.filter(r=>r.entryIndex>=DEV&&r.entryIndex<PRE);
  const report={strategy:'Strategy A',mode:'DELAY1_EXCURSION_BUDGET_OUTCOME',timeframe:tf,scope:{preHoldoutCandles:PRE,devCandles:DEV,valCandles:PRE-DEV,delayExactly:1,freshHoldoutExcluded:true},methodology:{buckets:BUCKETS.map(([name,minR,maxR])=>({name,minR,maxR})),favMilestones:FAV,horizons:H,noOptimization:true,noFreshHoldout:true,productionUntouched:true,note:'Adverse budget is measured from entry until each first favorable milestone. Trades that never reach that milestone are excluded from that milestone table. Same-bar OHLC ordering remains ambiguous.'},DEV:summarize(dev),VAL:summarize(val),allPreHoldout:summarize(rows),rows};
