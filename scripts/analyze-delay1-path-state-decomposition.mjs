...
async function run(){
 ...
 const delay1=usable.filter(t=>Number(t.entryIndex)>0);
 const rows=[];
 for(const t of delay1){
   ...
   for(let h=1;h<=H;h++){
     ...
     const net=(t.direction==='BUY'?c.close-entry:entry-c.close)/risk;
     path.push({h,mae,mfe,net});
   }
   if(path.length!==H)continue;
   const early={};
   for(const h of [3,5,10]){const p=path.slice(0,h);early[`t${h}`]={mae:Math.max(...p.map(x=>x.mae)),mfe:Math.max(...p.map(x=>x.mfe)),net:p[p.length-1].net};}
   const stateAt={};
   ...
   rows.push({entryIndex:i,direction:t.direction,rMultiple:Number(t.rMultiple),exceptional:Number(t.rMultiple)>=5,path,early,stateAt,laterRecovery});
 }
 const indices=[...new Set(rows.map(r=>r.entryIndex))];
 if(indices.length!==144)throw new Error(`Expected 144 DELAY1 rows from baseline replay, got ${indices.length}.`);
 const stateTables={};
 for(const h of [3,5,10]){
   stateTables[`T${h}`]={};
   for(const s of ['S0_LT25','S1_25_50','S2_50_75','S3_75_100','S4_GE1']){
     const all=rows.filter(r=>r.stateAt[`t${h}`]===s),dev=all.filter(r=>r.entryIndex<DEV),val=all.filter(r=>r.entryIndex>=DEV&&r.entryIndex<PRE),normal=all.filter(r=>!r.exceptional);
     stateTables[`T${h}`][s]={n:all.length,devN:dev.length,valN:val.length,all:stats(all.map(r=>r.rMultiple)),noExceptional:stats(normal.map(r=>r.rMultiple)),dev:stats(dev.map(r=>r.rMultiple)),val:stats(val.map(r=>r.rMultiple)),exceptional:all.filter(r=>r.exceptional).length,laterMFE:stats(all.map(r=>r.laterRecovery[`t${h}`].mfe)),laterMAE:stats(all.map(r=>r.laterRecovery[`t${h}`].mae))};
   }
 }
 const transitions={};
 for(const [a,b] of [[3,5],[3,10],[5,10]]){
   const table={};
   for(const s of ['S0_LT25','S1_25_50','S2_50_75','S3_75_100','S4_GE1']){
     const subset=rows.filter(r=>r.stateAt[`t${a}`]===s),counts={};
     for(const r of subset){const z=r.stateAt[`t${b}`];counts[z]=(counts[z]??0)+1;}
     table[s]={n:subset.length,counts};
   }
   transitions[`T${a}_TO_T${b}`]=table;
 }
 const direction={};
 for(const d of ['BUY','SELL']){
   const rs=rows.filter(r=>r.direction===d);
   direction[d]={n:rs.length,dev:rs.filter(r=>r.entryIndex<DEV).length,val:rs.filter(r=>r.entryIndex>=DEV).length,outcome:stats(rs.map(r=>r.rMultiple)),t3MAE:stats(rs.map(r=>r.early.t3.mae)),t5MAE:stats(rs.map(r=>r.early.t5.mae)),t10MAE:stats(rs.map(r=>r.early.t10.mae))};
 }
 const report={strategy:'Strategy A',mode:'DELAY1_PATH_STATE_DECOMPOSITION',timeframe:'5min',scope:{preHoldoutCandles:PRE,devCutoff:DEV,pathHorizon:H,stateHorizons:[3,5,10],fixedStateBands:['<0.25R','0.25-0.50R','0.50-0.75R','0.75-1.00R','>=1.00R'],freshHoldoutAccessed:false},integrity:{baselinePre:usable.length,delay1:144,matched:rows.length,rows:rows.length,devN:91,valN:53,pathComplete:true,deterministic:true},methodology:{purpose:'Descriptive decomposition of early cumulative MAE states, later recovery, and state transitions using fixed ex-ante bands.',statesFixedExAnte:true,noThresholdOptimization:true,noHorizonSelection:true,noExitRuleCreation:true,noBrokerExecutionModel:true,baselineOutcomeRetainedSeparately:true,freshHoldoutExcluded:true,productionUntouched:true},stateTables,transitions,direction};
 await mkdir(OUT,{recursive:true});await writeFile(resolve(OUT,'5m.json'),JSON.stringify(report,null,2));
 console.log(`PHASE_11_PATH_STATE_DECOMPOSITION N=${rows.length} DEV=91 VAL=53 FRESH=LOCKED`);
 console.log(`INTEGRITY baselinePre=${usable.length} delay1=144 matched=${rows.length} pathComplete=true deterministic=true`);
 ...
 console.log('=== DIRECTION ===');for(const [d,x] of Object.entries(direction))console.log(`${d} N=${x.n} DEV=${x.dev} VAL=${x.val} avgR=${x.outcome.avgR?.toFixed(4)??'NA'} | t3MAE=${x.t3MAE.avgR?.toFixed(4)??'NA'} t5MAE=${x.t5MAE.avgR?.toFixed(4)??'NA'} t10MAE=${x.t10MAE.avgR?.toFixed(4)??'NA'}`);
