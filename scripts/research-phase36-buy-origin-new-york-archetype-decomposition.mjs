...
const archetypes=['LOSS_A','LOSS_B','LOSS_C'];
const report={strategy:BASELINE.strategy,mode:'PHASE36_BUY_ORIGIN_NEW_YORK_ARCHETYPE_DECOMPOSITION',timeframe:'5min',baselineCommit:BASELINE_COMMIT,generationParity:{stored:stored.length,generated:generated.length,exact},mapping:{mappedBuyNonDNewYork:mapped,sourcePhase24Cases:phase24Cases.length},overall:stats(rows),archetypes:archetypes.map(a=>group(a,x=>x.archetype===a)),cross:archetypes.map(a=>group(`${a}_SPIKE_HIGH`,x=>x.archetype===a&&x.spikeSizeR>=PH27.interactionConditioning[0].thresholds.spikeSizeR)).concat(archetypes.map(a=>group(`${a}_CORR_EFF_HIGH`,x=>x.archetype===a&&x.correctionEfficiency>=PH27.interactionConditioning[1].thresholds.correctionEfficiency))),methodology:{researchOnly:true,noOptimization:true,noTradingRules:true,archetypeSource:'Phase24 post-entry loss archetype attribution; used descriptively only.',scope:'Original BUY, NON-D, NEW_YORK; counterfactual direction flip with preserved absolute risk/reward.',thresholdPolicy:'Only fixed Phase27 q66 thresholds; no threshold search.'},status:'COUNTERFACTUAL_ONLY_DECOMPOSITION'};
