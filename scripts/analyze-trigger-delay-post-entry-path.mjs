...
function stats(rows){const rs=rows.map(x=>x.r).filter(Number.isFinite),w=rs.filter(x=>x>0),l=rs.filter(x=>x<0),gp=w.reduce((a,b)=>a+b,0),gl=-l.reduce((a,b)=>a+b,0);return{n:rs.length,wins:w.length,losses:l.length,winRate:rs.length?w.length/rs.length:0,avgR:rs.length?rs.reduce((a,b)=>a+b,0)/rs.length:0,PF:gl?gp/gl:null};}
...
function delayFor(c){return c.entryIndex-c.correction.correctionExtremeIndex;}
function thresholdEvent(candles,t,threshold,horizon){const risk=Math.abs(t.entry-t.stopLoss);if(!(risk>0))return'NONE';const end=Math.min(candles.length-1,t.entryIndex+horizon);for(let i=t.entryIndex+1;i<=end;i++){const c=candles[i],fav=t.direction==='BUY'?c.high>=t.entry+threshold*risk:c.low<=t.entry-threshold*risk,adv=t.direction==='BUY'?c.low<=t.entry-threshold*risk:c.high>=t.entry+threshold*risk;if(fav&&adv)return'SAME_BAR';if(fav)return'FAVORABLE_FIRST';if(adv)return'ADVERSE_FIRST';}return'NONE';}
