...
async function run(timeframe){
  const src=JSON.parse(await readFile(resolve(ROOT,`data/reports/strategy-a-entry-geometry-forensics/${timeframe}.json`),'utf8'));
  const rows=classify((src.tradeRows||[]).filter(r=>Number.isFinite(r.rMultiple)).sort((a,b)=>new Date(a.entryTime)-new Date(b.entryTime)));
  const pairs=[];
  ...
  const report={
    strategy:'Strategy A / SP2L',mode:'RESEARCH_ENTRY_GEOMETRY_COMBINATION_FORENSICS',timeframe,
    scope:'All baseline trades with geometry features; exhaustive pairwise bucket combinations; chronological half split; maximum two features; no production rule changes',
    methodology:{
      source:'strategy-a-entry-geometry-forensics tradeRows',
      search:'all unordered pairs across the eight pre-declared geometry features and their fixed research buckets',
      complexity:'maximum two features; no three-way combinations or continuous threshold search',
      split:'first half DEV, second half OOS, chronological by entryTime',
      candidateGate:`OOS n >= ${MIN_OOS_N}, PF >= 1, avgR > 0`,
      nextGate:'Any candidate passing this gate is hypothesis-only and must be frozen before third chronological holdout testing.',
      warning:'pairwise search creates multiple-comparison risk; high OOS PF alone is not evidence of a real edge.'
    },
    coverage:{classifiedTrades:rows.length,pairCombinations:pairs.length},
    overall:{...summarize(rows),maxDD:dd(rows)},
    topOOS:eligible.slice(0,TOP_N),
    eligibleCount:eligible.length,
    allPairResults:pairs,
    nextResearchQuestion:'Freeze only a small number of pre-specified top candidates, then evaluate each on untouched chronological thirds (development/validation/holdout) and by BUY/SELL/session/day robustness.'
  };
