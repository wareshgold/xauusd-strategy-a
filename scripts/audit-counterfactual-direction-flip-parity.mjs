...
function simulate(entryIndex,entry,sl,tp1,tp2,direction){
 const risk=Math.abs(entry-sl);
 if(risk<=0)throw new Error('Counterfactual risk must be positive');
 for(let i=entryIndex+1;i<candles.length;i++){
  const c=candles[i],high=Number(c.high),low=Number(c.low);
  const hitSL=direction==='BUY'?low<=sl:high>=sl;
  const hitTP1=direction==='BUY'?high>=tp1:low<=tp1;
  const hitTP2=tp2===undefined||tp2===null?false:(direction==='BUY'?high>=tp2:low<=tp2);
  if(hitSL&&(hitTP1||hitTP2))return {result:'AMBIGUOUS',exitIndex:i,r:null};
  if(hitSL)return {result:'SL',exitIndex:i,r:-1};
  if(hitTP2)return {result:'TP2',exitIndex:i,r:Math.abs(tp2-entry)/risk};
  if(hitTP1)return {result:'TP1',exitIndex:i,r:Math.abs(tp1-entry)/risk};
 }
 return {result:'OPEN',exitIndex:null,r:null};
}
...
for(const t of canonical){
 const entryIndex=Number(t.entryIndex),entry=Number(t.entry),sl=Number(t.stopLoss),tp1=Number(t.tp1),tp2=t.tp2==null?undefined:Number(t.tp2),direction=t.direction;
 const original=simulate(entryIndex,entry,sl,tp1,tp2,direction);
 const risk=Math.abs(entry-sl),flipDirection=direction==='BUY'?'SELL':'BUY';
 const flipSL=flipDirection==='BUY'?entry-risk:entry+risk;
 const tp1Reward=Math.abs(tp1-entry),flipTP1=flipDirection==='BUY'?entry+tp1Reward:entry-tp1Reward;
 const tp2Reward=tp2===undefined?undefined:Math.abs(tp2-entry),flipTP2=tp2Reward===undefined?undefined:(flipDirection==='BUY'?entry+tp2Reward:entry-tp2Reward);
 const flipped=simulate(entryIndex,entry,flipSL,flipTP1,flipTP2,flipDirection);
 rows.push({entryIndex,entryTime:t.entryTime,direction,storedResult:t.result,storedR:Number(t.rMultiple),replayedResult:original.result,replayedR:original.r,originalParity:equivalent({result:t.result,r:Number(t.rMultiple)},original),flipDirection,flipSL,flipTP1,flipTP2,flippedResult:flipped.result,flippedR:flipped.r,flipExitIndex:flipped.exitIndex});
}

const originalMismatches=rows.filter(x=>!x.originalParity);
const flipResolved=rows.filter(x=>Number.isFinite(x.flippedR)),flipWins=flipResolved.filter(x=>x.flippedR>0),flipLosses=flipResolved.filter(x=>x.flippedR<=0),flipAmbiguous=rows.filter(x=>x.flippedResult==='AMBIGUOUS'),flipOpen=rows.filter(x=>x.flippedResult==='OPEN');
...
const report={strategy:'Strategy A / SP2L',mode:'COUNTERFACTUAL_DIRECTION_FLIP_PARITY_AUDIT',timeframe:'5min',canonical:canonical.length,originalReplay:{mismatches:originalMismatches.length,expected:canonical.length,actual:canonical.length,parityRate:canonical.length?1-originalMismatches.length/canonical.length:null},flip:{resolved:flipResolved.length,ambiguous:flipAmbiguous.length,openEnd:flipOpen.length,wins:flipWins.length,losses:flipLosses.length,byOriginalDirection:{BUY:summary(byOriginalDirection.BUY),SELL:summary(byOriginalDirection.SELL)}},methodology:'Exact mirror of BacktestEngine semantics: replay starts at entryIndex+1; if SL and TP1/TP2 occur on the same candle the result is AMBIGUOUS; otherwise SL is checked before TP2, then TP1. Counterfactual mirrors both TP1 and TP2 distances across the entry price. The original replay is the validation gate: if stored baseline outcomes cannot be reproduced, the flip result is not accepted.',status:originalMismatches.length===0?'PARITY_CONFIRMED_COUNTERFACTUAL_REVIEW_REQUIRED':'PARITY_FAILED_DO_NOT_TRUST_FLIP',mismatchExamples:originalMismatches.slice(0,20),rows};
...
console.log(`PHASE_COUNTERFACTUAL_DIRECTION_FLIP_PARITY 5min CANONICAL=${canonical.length} ORIGINAL_MISMATCH=${originalMismatches.length} FLIP_RESOLVED=${flipResolved.length} FLIP_AMBIGUOUS=${flipAmbiguous.length} FLIP_OPEN_END=${flipOpen.length}`);
