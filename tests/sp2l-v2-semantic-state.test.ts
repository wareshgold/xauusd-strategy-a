...
const bullishSetup: Sp2lEvent[] = [
  ...
  { type: 'STRUCTURAL_REFERENCE_IDENTIFIED', index: 12, price: 2500, status: 'SOURCE_CONFIRMED', rationale: 'Fixture structural low; selection remains source/TBD.' },
  { type: 'CORRECTION_BEGAN', index: 13 },
  { type: 'PENDING_LIMIT_CREATED', index: 13, entryPrice: 2500, stopLoss: 2492, entryStatus: 'CANDIDATE', stopStatus: 'CANDIDATE', rationale: 'Fixture only; no optimization.' },
];
...
describe('SP2L V2 semantic state model (non-production)', () => {
  ...
  it('rejects a strong move without context', () => {
    const state = replay([{ type: 'STRONG_MOVE_STARTED', impulseId: 'impulse-1', direction: 'BULLISH' }]);
  ...
  it('cancels an untouched pending order on structural invalidation', () => {
    const state = applySp2lEvent(replay(bullishSetup), { type: 'STRUCTURAL_INVALIDATION', index: 14 });
  ...
  it('records a post-fill stop as a stop outcome, not as pending invalidation', () => {
    const filled = applySp2lEvent(replay(bullishSetup), { type: 'LIMIT_TOUCHED', index: 15, price: 2500 });
    const stopped = applySp2lEvent(filled, { type: 'STOP_HIT', index: 16, price: 2492 });
  ...
  it('ignores a non-matching post-fill stop price', () => {
    const filled = applySp2lEvent(replay(bullishSetup), { type: 'LIMIT_TOUCHED', index: 15, price: 2500 });
    const unchanged = applySp2lEvent(filled, { type: 'STOP_HIT', index: 16, price: 2491 });
  ...
  it('keeps Leg 1 and Leg 2 geometry explicit instead of inventing formulas', () => {
    ...
    expect(state.geometry.leg2ProjectionOrigin.concept).toBeNull();
  ...
  it('keeps an unresolved Leg 2 origin separate from the pending fill price', () => {
    ...
    expect(filled.geometry.leg2ProjectionOrigin.concept).toBeNull();
  });

  it('accepts a source-shaped correction-origin classification without equating it to fill', () => {
    const classified = applySp2lEvent(replay(bullishSetup), {
      type: 'LEG2_PROJECTION_ORIGIN_IDENTIFIED',
      index: 14,
      price: null,
      status: 'SOURCE_CONFIRMED',
      concept: 'OTHER_VISUAL_POINT',
      rationale: 'Parent Leg 2 begins from the intervening deep correction; exact OHLC coordinate remains unresolved.',
    });
    ...
    expect(classified.geometry.leg2ProjectionOrigin.concept).toBe('OTHER_VISUAL_POINT');
  ...
  it('rejects a Leg 2 origin placed before the correction begins', () => {
    const invalid = applySp2lEvent(replay(bullishSetup), {
      type: 'LEG2_PROJECTION_ORIGIN_IDENTIFIED',
      index: 12,
      price: 2498,
      status: 'CANDIDATE',
  ...
  it('keeps 2X separate from position 1', () => {
    const state = applySp2lEvent(replay(bullishSetup), { type: 'LIMIT_TOUCHED', index: 15, price: 2500 });
    expect(state.position.position2xEnabled).toBe(false);
  });
  ...
  it('measures explicit leg endpoints without selecting them or fitting tolerance', () => {
    const measurement = measureSp2lLegs({ leg1StartPrice: 2500, leg1EndPrice: 2520, leg2StartPrice: 2508, leg2EndPrice: 2528 });
  ...
  it('returns an undetermined equality relation when Leg 1 has zero magnitude', () => {
    const measurement = measureSp2lLegs({ leg1StartPrice: 2500, leg1EndPrice: 2500, leg2StartPrice: 2508, leg2EndPrice: 2528 });
  ...
  it('rejects non-finite explicit endpoint prices', () => {
    expect(() => measureSp2lLegs({ leg1StartPrice: Number.NaN, leg1EndPrice: 2520, leg2StartPrice: 2508, leg2EndPrice: 2528 })).toThrow('LEG1_START_PRICE_MUST_BE_FINITE');
  });
});
