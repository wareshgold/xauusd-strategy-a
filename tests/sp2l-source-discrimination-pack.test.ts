...
import {FIXTURES,discriminationPack,assertNoCanonicalSelection} from "../research/harness/sp2l_source_discrimination_pack_v1.js";
describe("SP2L source discrimination pack",()=>{
it("covers F10/F11/F12/F14",()=>{expect(new Set(FIXTURES.map((f: typeof FIXTURES[number])=>f.blocker))).toEqual(new Set(["F10","F11","F12","F14"]));expect(FIXTURES.length).toBe(9);});
